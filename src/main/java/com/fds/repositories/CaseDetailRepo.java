...
@Repository
public interface CaseDetailRepo extends JpaRepository<FdsCaseDetail, String>, CaseDetailRepoCustom {

	public static final String TABLE_NAME = "FDS_CASE_DETAIL";
	...
	@Query(value = "select f from FdsCaseDetail f where f.checkNew = :ischecknew")
	Page<FdsCaseDetail> findAllBycheckNewIs(@Param(value = "ischecknew") String ischecknew, Pageable page);
	
	@Query(value = "select f from FdsCaseDetail f where f.checkNew = :ischecknew and f.mercName NOT LIKE %:merc%")
	Page<FdsCaseDetail> findAllBycheckNewIsAndNotMerc(@Param(value = "ischecknew") String ischecknew,@Param(value = "merc") String merc, Pageable page);

	@Query(value = "select f from FdsCaseDetail f where f.checkNew = :ischecknew and f.mercName LIKE %:merc%")
	Page<FdsCaseDetail> findAllBycheckNewIsAndMerc(@Param(value = "ischecknew") String ischecknew,@Param(value = "merc") String merc, Pageable page);
	 ...
	 * @see Inbox
	 */
	Page<FdsCaseDetail> findByUsrIIgnoreCasedAndCaseStatusIn(String userid, String[] casestatus, Pageable page);
	
	Page<FdsCaseDetail> findByCaseStatusIn(String[] casestatus, Pageable page);
	
	Page<FdsCaseDetail> findByUsrIIgnoreCasedAndCaseStatusInAndAutoClose(String userid, String[] casestatus, String autoClose, Pageable page);
	
	Page<FdsCaseDetail> findByCaseStatusInAndAutoClose(String[] casestatus, String autoClose, Pageable page);
	/**
	 ...
	 * @see ClosedCase
	 */
	@Query(value = "select f from FdsCaseDetail f where f.caseStatus in (:casestatus) and f.usrId in (:listofuserid)")
	Page<FdsCaseDetail> findByUsrListAndCaseStatus(@Param(value = "listofuserid") String[] listofuserid,@Param(value = "casestatus") String[] casestatus, Pageable page);
	 ...
	 * @return Page<FdsCaseDetail>
	 */
	Page<FdsCaseDetail> findAllBycaseStatus(@Param("status") String status, Pageable page);
	 ...
	 */
	FdsCaseDetail findOneByCaseNo(@Param(value = "caseno") String caseno);
...
//	@Query(value = "select ccps.ded2A@im(:cardno,'FDS') from dual", nativeQuery = true)
	...
	@Query(value ="SELECT CRE_TMS, UPD_TMS, UPD_UID, ASG_TMS, INIT_USR_ID, :userRecord USR_ID, CIF_NO, " + 
			...
			"FROM FDS_CASE_DETAIL A INNER JOIN FDS_SYS_TASK B ON A.CIF_NO = B.OBJECTTASK " + 
	...
	public List<FdsCaseDetail> findCaseByMerchantSystask(@Param("userRecord") String userRecord);
	
	//@formatter:on
	@Modifying
	@Query(value = "UPDATE FDS_CASE_DETAIL\r\n" + 
			"SET CASE_STATUS='DIC',ASG_TMS=(select to_char(SYSDATE, 'yyyyMMddHH24MISS') from dual),\r\n" + 
			"UPD_TMS=(select to_char(SYSDATE, 'yyyyMMddHH24MISS') from dual),\r\n" + 
			"USR_ID=:user,CHECK_NEW= ' '\r\n" + 
			"WHERE CRE_TMS BETWEEN :fromdate AND :todate\r\n" + 
			"AND CHECK_NEW='Y'",nativeQuery = true)
	int updateStatusCaseClear(@Param(value = "user") String user,@Param(value = "fromdate") BigDecimal fromdate,@Param(value = "todate") BigDecimal todate);
