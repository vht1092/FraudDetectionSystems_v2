...
public interface TxnDetailRepo extends CrudRepository<FdsTxnDetail, FdsTxnDetailPK> {
	...
	@Query(nativeQuery = true, value = "select FX_SSPTXN_TOKEN from {h-schema}fds_txn_detail inner join DW_SSP_TXN on FX_OA008_USED_PAN = FX_SSPTXN_PAN and P9_OA008_SEQ = F9_SSPTXN_SEQ where FX_OA008_USED_PAN = ?1 and F9_OA008_CRE_TMS = ?2 AND ROWNUM<=1")
