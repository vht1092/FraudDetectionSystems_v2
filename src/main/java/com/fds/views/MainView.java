...
public class MainView extends HorizontalLayout implements View {
	...
	@Autowired
	private transient SysTaskService sysTaskService;
	@Autowired
	private transient CaseDetailService caseDetailService;
	...
	void init() {
		...
		Page.getCurrent().setTitle("Fraud Detection System");
		setSizeFull();
...
//			}
	...
	private void addNewTab(final String itemName) {
		try {
			final Class<? extends Component> tabType = FdsTabType.getTabType(itemName).getClassComponent();
...
//		final Label title = new Label("<img class=\"v-icon\" src=\"" + urlServer + "/VAADIN/themes/mytheme/img/logo_inside.png\"></img>");
		final Label title = new Label("<img class=\"v-icon\" src=\"" + urlServer + "/VAADIN/themes/mytheme/img/logo_inside_new.png\" style='width: 215px;height: 90px;'></img>");
		...
		if (SecurityUtils.isLoggedIn()) {
			getUI().addPollListener(new UIEvents.PollListener() {
				...
				public void poll(UIEvents.PollEvent event) {
					caseDetailService.updateTaskCase();
					refreshTabContent();					
				}
			});
		} else {
