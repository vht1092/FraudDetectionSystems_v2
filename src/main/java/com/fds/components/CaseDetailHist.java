...
import com.fds.services.CaseStatusHistService;
...
@SpringComponent
@Scope("prototype")
public class CaseDetailHist extends CustomComponent implements CaseRegister {

	private static final long serialVersionUID = 1L;

	private final transient CaseDetailHistService caseDetailService;
	private final transient CaseStatusHistService caseStatusService;
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(CaseDetailHist.class);
	...
	public CaseDetailHist(final String caseno) {
		super();
		this.sCaseno = caseno;

		final SpringContextHelper helper = new SpringContextHelper(VaadinServlet.getCurrent().getServletContext());
		caseDetailService = (CaseDetailHistService) helper.getBean("caseDetailHistService");
		caseStatusService = (CaseStatusHistService) helper.getBean("caseStatusHistService");
	...
	private void createForm() {
		...
		if (listCaseDetail.isEmpty()) {
			...
			/* ----- NOI DUNG XU LY ----- */
			final List<FdsCaseStatusHist> listCaseStatus = caseStatusService.findAllByCaseNo(this.sCaseno);
			StringBuilder sHtmlComment = new StringBuilder("<table>");
			String action = "";
			for (final FdsCaseStatusHist a : listCaseStatus)
			{
...
//			verticalLayout.addComponent(btComment);
	...
	public String userProcesing() {
		final FdsCaseDetailHist fdsCaseDetail = caseDetailService.findOneByCaseNo(this.sCaseno);
