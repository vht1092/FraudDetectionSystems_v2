...
/**
 * Form chuyen case cho Case sang BOX "THEO DOI"
 * UPDATE STATUS = 'CAL'
 */
@SpringComponent
@Scope("prototype")
public class TransferForm extends VerticalLayout {
	...
	public TransferForm(final Callback callback, final String caseno, final String cardnumber) {
		...
		btSave.addClickListener(evt -> {
			...
			final String sComment = "".equals(caseCommentForm.getComment()) ? "Chuyển case để theo dõi" : caseCommentForm.getComment();
			/*if (cbboxUser.getValue() != null) {*/
				try {
					...
					caseDetailService.closeCase(sCaseNo, "ALL", STATUS);
					callback.closeWindow();
				} catch (Exception e) {
